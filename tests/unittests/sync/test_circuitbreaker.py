...
from purgatory.sync.circuitbreaker import SyncCircuitBreakerFactory
from tests.unittests.conftest import FakeRedis


def test_circuitbreaker_factory_decorator(
    sync_circuitbreaker: SyncCircuitBreakerFactory,
):

    count = 0

    @sync_circuitbreaker(circuit="client")
    ...
    brk = sync_circuitbreaker.uow.contexts.get("client")
    assert brk == Context(name="client", threshold=5, ttl=30)
    # assert sync_circuitbreaker.breakers["client"]._state._state.failure_count == 1

    @sync_circuitbreaker(circuit="client2", threshold=15)
    def _success2():
        pass

    _success2()
    brk = sync_circuitbreaker.uow.contexts.get("client2")
    assert brk == Context(name="client2", threshold=15, ttl=30)

    @sync_circuitbreaker(circuit="client3", ttl=60)
    def _success3():
        pass

    _success3()
    brk = sync_circuitbreaker.uow.contexts.get("client3")
    assert brk == Context(name="client3", threshold=5, ttl=60)


def test_redis_circuitbreaker_factory_decorator(
    sync_fake_redis: FakeRedis, sync_circuitbreaker_redis: SyncCircuitBreakerFactory
):

    count = 0
    sync_circuitbreaker_redis.initialize()

    @sync_circuitbreaker_redis(circuit="client", threshold=3, ttl=0.1)
    def fail_or_success(fail=False):
        if fail:
            raise RuntimeError("Boom")
        nonlocal count
    ...
    assert sync_fake_redis.deserialized_storage == {
        "cbr::client": {
            "name": "client",
            "opened_at": None,
            "state": "closed",
...
def test_circuitbreaker_factory_get_breaker(
    ...
    cbr,
    sync_circuitbreaker: SyncCircuitBreakerFactory,
    sync_circuitbreaker_redis: SyncCircuitBreakerFactory,
):
    cbreaker = {"inmemory": sync_circuitbreaker, "redis": sync_circuitbreaker_redis}[
        uow
    ]
    cbreaker.initialize()
...
def test_circuitbreaker_raise_state_changed_event(sync_circuitbreaker):
    ...
    sync_circuitbreaker.messagebus.add_listener(ContextChanged, evt_handler)
    brk = sync_circuitbreaker.get_breaker("my", threshold=2)
    try:
        with brk:
            raise RuntimeError("Boom")
    except RuntimeError:
