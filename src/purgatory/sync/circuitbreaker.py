...
from purgatory.sync.message_handlers import (
    sync_inc_circuit_breaker_failure,
    sync_register_circuit_breaker,
    sync_reset_failure,
    sync_save_circuit_breaker_state,
)
from purgatory.typing import TTL, CircuitName, Hook, Threshold

from .messagebus import SyncMessageRegistry
from .unit_of_work import SyncAbstractUnitOfWork, SyncInMemoryUnitOfWork
...
class SyncCircuitBreakerFactory:
    def __init__(
        ...
        self.messagebus.add_listener(
            CreateCircuitBreaker, sync_register_circuit_breaker
        )
        self.messagebus.add_listener(ContextChanged, sync_save_circuit_breaker_state)
        self.messagebus.add_listener(
            CircuitBreakerFailed, sync_inc_circuit_breaker_failure
        )
        self.messagebus.add_listener(CircuitBreakerRecovered, sync_reset_failure)
