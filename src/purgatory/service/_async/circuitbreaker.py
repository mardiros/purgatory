...
class AsyncCircuitBreaker:
    def __init__(
        self, context: Context, uow: AsyncAbstractUnitOfWork, messagebus: AsyncMessageRegistry
    ) -> None:
...
class PublicEvent:
    ...
    async def cb_created(self, event: CircuitBreakerCreated, uow: AsyncAbstractUnitOfWork):
    ...
    async def cb_failed(self, event: CircuitBreakerCreated, uow: AsyncAbstractUnitOfWork):
        self.hook(event.name, "failed", event)

    async def cb_recovered(self, event: CircuitBreakerCreated, uow: AsyncAbstractUnitOfWork):
